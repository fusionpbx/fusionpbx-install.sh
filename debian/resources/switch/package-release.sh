...
if [ ."$cpu_architecture" = ."arm" ]; then
        echo "deb https://repo.fusionpbx.com/armhf jessie $switch_version" > /etc/apt/sources.list.d/freeswitch.list
        curl https://repo.fusionpbx.com/public.key | apt-key add -
else
        if [ ."$os_codename" = ."stretch" ]; then
                echo "deb https://repo.fusionpbx.com/armhf stretch $switch_version" > /etc/apt/sources.list.d/freeswitch.list
                curl https://repo.fusionpbx.com/public.key | apt-key add -
        else
